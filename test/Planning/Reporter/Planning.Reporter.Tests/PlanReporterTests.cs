		...
		using( Assert.EnterMultipleScope() ) {
			...
			Assert.That( output, Does.Contain(
				string.Join(
					...
					"Has Shortfall,True",
					"First Shortfall Date,Jul 2069",
					"First Shortfall Period,523",
					"Shortfall Period Count,24",
					"Total Unfunded Shortfall,365848.02" ) ) );
			Assert.That( output, Does.Contain(
				string.Join(
					Environment.NewLine,
					"Tax Summary",
					"Total Federal Tax,677431.35",
					"Total Provincial Tax,335746.61",
					"Total Tax,1013177.96" ) ) );
		}
	}
	...
	public void WriteCalculatedToCsv_EscapesSpecialCharactersInNames() {
		Plan plan = TestPlanFactory.Create(
			...
			assets: [
				...
				TestPlanFactory.CreateAsset( "Non-Reg", AssetTaxStatus.CapitalGains, "Todd, \"TJ\"", 50m, hasUnlimitedContributionRoom: true ),
				...
				TestPlanFactory.CreateAsset( "Non-Reg", AssetTaxStatus.CapitalGains, "Tina", 50m, hasUnlimitedContributionRoom: true )
			],
